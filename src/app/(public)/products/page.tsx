...
const getNumberProperty = (obj: Record<string, unknown>, key: string): number => {
  const value = obj[key];
...
function ProductsContent() {
  ...
  const [collections, setCollections] = useState<Array<{id: string; name: string; slug: string; image_url: string; productCount: number}>>([]);
      ...
      try {
        ...
        if (response?.data) {
          const collectionsData = response.data.map((cat: unknown) => {
            const category = cat as { id: string | number; name: string; slug: string; productCount?: number };
            return {
  ...
  const formattedProducts = products.map((product: unknown) => {
    ...
    const productName = getStringProperty(productObj, 'name') || getStringProperty(productObj, 'product_name') || '';
    const productPrice = getNumberProperty(productObj, 'price') || getNumberProperty(productObj, 'product_price') || 0;
    const productImage = getStringProperty(productObj, 'image') || getStringProperty(productObj, 'image_url') || '';
    ...
    const metaDescription = getStringProperty(productObj, 'meta_description') || '';
    const updatedAt = getStringProperty(productObj, 'updated_at') || new Date().toISOString();
    const createdAt = getStringProperty(productObj, 'createdAt') || new Date().toISOString();
    ...
    const baseProduct: Product = {
      ...
      features: Array.isArray(productObj.features) ? productObj.features as string[] : [],
      tags: Array.isArray(productObj.tags) ? productObj.tags as string[] : [],
  ...
  const productData = formattedProducts.map(product => {
    ...
    const createdAt = typeof product.created_at === 'string' 
      ? product.created_at 
      : typeof product.createdAt === 'string' 
        ? product.createdAt 
        : new Date().toISOString();
      
    const updatedAt = typeof product.updated_at === 'string' 
      ? product.updated_at 
